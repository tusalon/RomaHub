            ...
            <div className="mt-4 px-1" data-name="searchbar-suggestions" data-file="components/SearchBar.js">
              ...
              <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar pb-1" data-name="province-suggestions" data-file="components/SearchBar.js">
                {provincias.slice(0, 8).map((item) => (
                  <button
                    key={item}
                    className={`px-3 py-1.5 rounded-full border text-xs whitespace-nowrap transition-colors ${provincia === item ? 'bg-[var(--primary-color)] text-white border-[var(--primary-color)]' : 'border-[var(--border)] bg-white text-[var(--text-muted)] hover:text-[var(--primary-color)] hover:border-[rgba(181,0,99,0.35)]'}`}
