-- RomaHub - Fase 3: métricas de conversión sin datos personales.

create table if not exists public.romahub_eventos (
  ...
  item_tipo text check (item_tipo is null or item_tipo in ('producto', 'curso', 'servicio')),
  item_id text,
  item_nombre text,
  created_at timestamptz not null default now()
);

create index if not exists romahub_eventos_negocio_fecha_idx
  on public.romahub_eventos (negocio_id, created_at desc);

create index if not exists romahub_eventos_tipo_fecha_idx
  on public.romahub_eventos (evento, created_at desc);

alter table public.romahub_eventos enable row level security;

comment on table public.romahub_eventos is
  'Eventos agregados de RomaHub. No almacena IP, cookies, WhatsApp ni datos personales de visitantes.';
...
begin
  ...
  into v_visitas, v_whatsapp, v_reservas, v_productos, v_compartidos
